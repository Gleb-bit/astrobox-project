...
class Brain:
    ...
    def updatet_asteroids(self, asteroids):
        ...
        XvatovDrone.brain.asteroids_not_empty = []
        for i, asteroid in enumerate(asteroids):
            if asteroid.payload != 0:
                xi, yi = asteroid.coord.x, asteroid.coord.y
                li = (xi ** 2 + yi ** 2) ** (0.5)
                if asteroid.payload != 0:
                    XvatovDrone.brain.asteroids_not_empty.append([i, li, asteroid.payload])
        if XvatovDrone.brain.asteroids_not_empty:
            XvatovDrone.brain.asteroids_not_empty.sort(key=lambda x: x[1])
            return XvatovDrone.brain.asteroids_not_empty
        else:
            return []

    def get_asretoid_after_born(self, person):
        if not self.asteroids_not_empty:
    ...
    def get_place_near_mothership(self, person):
        ...
        if person.my_mothership.coord.x <= theme.FIELD_WIDTH // 2 and person.my_mothership.coord.y <= theme.FIELD_HEIGHT // 2:
            y = list(range(y[0], y[-1], -dy))[:self.army_quantity]
            x = list(range(x[0], x[-1], dx))[:self.army_quantity]
    ...
    def next_action(self, person):
        ...
        if not person.fight:
            ...
            if len(self.get_bases(person)) == 0 and len(self.get_enemies(person)) == 0:
                ...
                my_position_defense = self.position_defense[person.number_drone]
                if person.health < 50:
                    person.move_at(person.my_mothership)
                else:
                    if len(self.get_enemies(person)) == 0:
                        ...
                        if not person.near(my_position_win):
                            ...
                            person.shoot(target)
                    elif len(self.get_enemies(person)) == 1 and self.army_quantity>3:
