...
class DmitrievaDrone(Drone):
    ...
    def terminator_act(self):
        ...
        if not self.target:
            ...
            self.get_role()
        elif self.near(position) and self.can_shoot(self.target):
            self.turn_to_and_shot_target()
        else:
    ...
    def can_shoot(self, target, save_distance=50):
        ...
        r = save_distance
        return self._no_teammates_at_gunpoint(x0, y0, x, y, r)
    ...
    def check_drone_health(self, safe_share_of_health=0.6):
        ...
        elif 0 < self.health < theme.DRONE_MAX_SHIELD * safe_share_of_health and \
                not self.near(self.get_position_circle()):
            self.choose_target_and_move_at(self.my_mothership)
